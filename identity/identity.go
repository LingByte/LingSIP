...
// trustDomains lists upstream signaling peers from which we accept
// (and forward) P-Asserted-Identity. Install via ConfigureTrustDomains
// during SIP server bootstrap (server.Config.PAITrustDomains).
//
// Each entry is a bare host or host:port (lowercased on ingest). Empty
// list means "accept PAI from any peer" — appropriate for dev / loopback,
// NOT for production carrier interop where you should pin the carrier SBC.
var (
	trustDomainsMu   sync.RWMutex
	trustDomainsList []string
)

// ConfigureTrustDomains installs the PAI trust-domain allow-list. Safe to
// call again (e.g. tests); pass nil or empty to trust any peer.
func ConfigureTrustDomains(domains []string) {
	var out []string
...
// SetTrustDomainsForTest overrides the trust list in unit tests.
func SetTrustDomainsForTest(list []string) {
	ConfigureTrustDomains(list)
}
