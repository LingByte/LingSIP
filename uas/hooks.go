...
func wrapInvite(h InviteHandler) stack.HandlerFunc {
	...
	return func(req *stack.Message, addr *net.UDPAddr) *stack.Message {
		resp, err := h(req, addr)
		if err != nil {
			r, e2 := ErrorResponse(req, 500, ReasonServerInternalError)
			if e2 != nil || r == nil {
				return nil
			}
			return r
...
func wrapSimple(h SimpleHandler) stack.HandlerFunc {
	...
	return func(req *stack.Message, addr *net.UDPAddr) *stack.Message {
		resp, err := h(req, addr)
		if err != nil {
			r, _ := ErrorResponse(req, 500, ReasonServerInternalError)
			return r
		}
		return resp
	}
...
func defaultOptions(req *stack.Message, addr *net.UDPAddr) (*stack.Message, error) {
	...
	resp, err := NewResponse(req, stack.StatusOK, stack.ReasonOK, "", "")
	if err != nil {
		return nil, err
	}
	resp.SetHeader(stack.HeaderAllow, allow)
	resp.SetHeader(stack.HeaderAccept, stack.ContentTypeSDP)
	return resp, nil
}
