...
func TestRunInviteClientReliable_SingleSend(t *testing.T) {
	mgr := NewManager()
	inv := testInvite(t)
	remote := &net.UDPAddr{IP: net.IPv4(127, 0, 0, 1), Port: 9}
	...
	res, err := mgr.RunInviteClientReliable(ctx, inv, remote, send, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	if sends.Load() != 1 {
		t.Fatalf("reliable INVITE must send once, got %d", sends.Load())
	}
}
