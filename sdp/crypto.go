...
// CryptoOffer represents a single a=crypto line from SDP (RFC 4568),
// associated with an audio or video media line.
type CryptoOffer struct {
	Tag       uint32 // Crypto attribute tag (1,2,3...)
	Suite     string // SRTP encryption suite (e.g. AES_CM_128_HMAC_SHA1_80)
	KeyParams string // Raw key parameters (e.g. "inline:ABC123...")
}
...
func DecodeSDESInline(keyParams string) (masterKey, masterSalt []byte, err error) {
	...
	if keyParams == "" {
		return nil, nil, ErrEmptyCryptoKeyParams
	}
	...
	if !strings.HasPrefix(strings.ToLower(inline), "inline:") {
		return nil, nil, ErrCryptoMissingInline
	}
	...
	if err != nil {
		return nil, nil, errInlineBase64(err)
	}
	...
	if len(raw) < kl+sl {
		return nil, nil, errInlineMaterialTooShort(len(raw), kl+sl)
	}
...
func FormatCryptoLine(tag uint32, suite string, masterKey, masterSalt []byte) (string, error) {
	...
	if len(masterKey) != kl || len(masterSalt) != sl {
		return "", errKeySaltLengthMismatch(suite)
	}
