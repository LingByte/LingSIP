...
func staticPayloadCodec(pt uint8) (Codec, bool) {
	switch pt {
	case 0:
		return Codec{PayloadType: 0, Name: CodecPCMU, ClockRate: 8000, Channels: 1}, true
	case 8:
		return Codec{PayloadType: 8, Name: CodecPCMA, ClockRate: 8000, Channels: 1}, true
	case 9:
		return Codec{PayloadType: 9, Name: CodecG722, ClockRate: 8000, Channels: 1}, true
	default:
		return Codec{}, false
	}
}
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(line, "m=audio") {
			...
			if len(m) >= 4 {
				...
				if err != nil {
					return nil, errInvalidAudioPort(err)
				}
				info.Port = port
		...
		if strings.HasPrefix(line, "a=rtpmap:") {
			...
			if len(m) >= 4 {
				...
				name := strings.ToLower(strings.TrimSpace(m2[2]))
...
func GenerateWithProtoExtras(localIP string, localPort int, proto string, codecs []Codec, extraLines []string) string {
	...
	for _, c := range codecs {
		...
		if strings.EqualFold(c.Name, CodecOpus) {
			b.WriteString(fmt.Sprintf("a=fmtp:%d minptime=10;useinbandfec=1\r\n", c.PayloadType))
		}
		if strings.EqualFold(c.Name, CodecTelephoneEvent) {
...
func PickTelephoneEventFromOffer(offer []Codec, matchClockRate int) (Codec, bool) {
	...
	for _, c := range offer {
		if !strings.EqualFold(strings.TrimSpace(c.Name), CodecTelephoneEvent) {
			continue
		}
