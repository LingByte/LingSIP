...
func NewUASFromINVITE(inv *stack.Message) (*Dialog, error) {
	if inv == nil || !inv.IsRequest || inv.Method != stack.MethodInvite {
		return nil, fmt.Errorf("sipdialog: need INVITE request")
	}
	callID := strings.TrimSpace(inv.GetHeader("Call-ID"))
	br := stack.TopBranch(inv)
	if callID == "" || br == "" {
		return nil, fmt.Errorf("sipdialog: missing Call-ID or Via branch")
	}
	n := stack.ParseCSeqNum(inv.GetHeader("CSeq"))
	if n <= 0 || !stack.IsInviteCSeq(inv) {
		return nil, fmt.Errorf("sipdialog: invalid INVITE CSeq")
	}
	d := &Dialog{
		CallID:       callID,
		InviteBranch: br,
...
// InviteTransactionKey returns the same key used by pkg/siptransaction for INVITE server/client maps.
func (d *Dialog) InviteTransactionKey() string {
	if d == nil {
		return ""
	}
	return stack.InviteTransactionKey(d.InviteBranch, d.CallID)
}
